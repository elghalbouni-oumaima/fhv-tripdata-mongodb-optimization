...
def make_heatmap(data, title):
    # Matrice de confusion simplifiée entre PULocationID et DOLocationID
...
def make_kpi_card(title, before_val, after_val, unit=""):
    ...
    color_class = "diff-good" if diff <= 0 else "diff-bad" # Supposons que moins c'est mieux (temps, docs scannés)
    if "Keys" in title and diff > 0: color_class = "diff-good" # Exception si on veut plus de clés utilisées ? Non, souvent moins de scan c'est mieux.
